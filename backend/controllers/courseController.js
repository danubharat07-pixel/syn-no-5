...
exports.createCourse = async (req, res) => {
  try {
    ...
    if (err.code === 11000) {
      return res.status(409).json({ success: false, message: 'courseName must be unique' });
    }
...
exports.updateCourse = async (req, res) => {
  try {
    ...
    if ('courseName' in req.body) {
      return res
        .status(400)
        .json({ success: false, message: 'Use PATCH /:id/rename to change courseName' });
    }
...
exports.renameCourse = async (req, res) => {
  try {
    ...
    if (course.courseName === newName.trim()) {
      return res.status(400).json({ success: false, message: 'New name is same as current name' });
    }
